...
export function MobileAttendancePunch({ config }: { config: MobileConfig }) {
  const [employeeIdentifier, setEmployeeIdentifier] = useState("EMP-002");
  ...
  const localTime = useMemo(() => new Intl.DateTimeFormat("en-SA", {
    timeZone: timezone,
    dateStyle: "medium",
    timeStyle: "medium",
    hour12: false
      ...
      await navigator.credentials.create({
        publicKey: {
          ...
          user: {
            ...
            displayName: employeeIdentifier
          },
  ...
  return (
    <div className="grid two">
      <section className="panel grid">
        ...
        {message ? <p className={message.includes("Unable") || message.includes("outside") || message.includes("mandatory") || message.includes("not") ? "status danger" : "status"}>{message}</p> : null}
      </section>
